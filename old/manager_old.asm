...
extern clock_speed          ;User-Defined function
;extern atof
...
initial_tics db "\nThe clock is now %ld tics and the computation will begin.", 10, 0
column_head db "\nTerms completed\tHarmonic sum", 10 , 0
row_hsum db "\t%ld\t\t%lf", 10, 0
final_tics db "\nThe clock is now %ld tics, which equals %f seconds.", 10, 0
exit_msg db "The harmonic sum will be returned to the driver.", 10, 0

dbg_clock_speed db "Clock Speed is %f GHz", 10, 0
...
call clock_speed
mov rax, 1
mov rdi, dbg_clock_speed
call printf
;; DEBUG clock_speed _END
...
;;	Receive input
push qword 0                     	;Reserve 16 bytes of storage for incoming number
push qword 0						;Reserve 16 bytes of storage for incoming number
...
pop r15                          	;First inputted integer is saved in r15
pop rax								;pop arbitrary value from stack
mov [input], r15								
;; Receive input _END
...
mov r14, qword [input]						; Initialize counter validation to inputted value
cvtsi2sd xmm15, qword [1]					; xmm15 = hsum numerator, always 1
